...
void fileRead(vector<string> &v, ifstream &fin) { // fin으로 부터 벡터 v에 읽어 드림
    string line;
    while (true) {
        getline(fin, line); // fin 파일에서 한 라인 읽기
        if (fin.eof()) break; // 끝까지 읽었음
        v.push_back(line); // 읽은 라인을 벡터에 저장
    }
}

void search(vector<string> &v, string word) { // 벡터 v에서 word 찾아 출력
...
int main() {
    ...
    while (true) {
        cout << "검색할 단어를 입력하시오 >> ";
        string word;
        getline(cin, word); // 키보드로 부터 문자열 읽기
        if (word == "exit")
            break; // 프로그램 종료
